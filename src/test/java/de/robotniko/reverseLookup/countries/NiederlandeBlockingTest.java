...
public class NiederlandeBlockingTest {
	...
	public void test1() throws ReverseLookupException, ParserConfigurationException, SAXException, IOException {
		ReverseLookupRequest request = new ReverseLookupRequest("+31117371233");
		...
		Assert.assertEquals("Nieuwvliet", result.getCity());
	}

	@Test
	public void test2() throws ReverseLookupException, ParserConfigurationException, SAXException, IOException {
		ReverseLookupRequest request = new ReverseLookupRequest("+31207711969");
		...
		ReverseLookupResponse result = results.get(0);
		Assert.assertNull(result.getCompany());
		Assert.assertEquals("C", result.getFirstName());
		Assert.assertEquals("Smid", result.getLastName());
		Assert.assertEquals("Westerdok 328", result.getStreet());
		Assert.assertNull(result.getHouseNumber());
		Assert.assertEquals("1013BH", result.getZipCode());
		Assert.assertEquals("Amsterdam", result.getCity());
	}
